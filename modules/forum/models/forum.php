...
class m_forum_m_forum extends Model
{
	...
	public function get_topics($forum_id)
	{
		...
		foreach ($topics as &$topic)
		{
			...
			$unread = $this->user->id && $forum_read < $last_message_date && (!isset($topics_read[$topic['topic_id']]) || $topics_read[$topic['topic_id']] < $last_message_date);
	...
	{
		if (!$forum['count_topics'] || !$this->user->id)
		{
			return FALSE;
		}
