		...
		<tbody class="forum-content">
			...
			<tr>
				...
				<td class="text-left col-md-9 col-sm-10">
					...
					<a href="<?php echo url($this->session->get_back() ?: 'forum/'.$forum_id.'/'.url_title($title)); ?>" class="btn btn-default"><?php echo $this->lang('back'); ?></a>
					<button type="submit" class="btn btn-primary"><?php echo $this->lang('post_topic'); ?></button>
					<?php elseif (!empty($topic_id)): ?>
					<a href="<?php echo url($this->session->get_back() ?: 'forum/topic/'.$topic_id.'/'.url_title($title)); ?>" class="btn btn-default"><?php echo $this->lang('back'); ?></a>
