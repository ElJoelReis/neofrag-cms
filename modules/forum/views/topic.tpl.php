		...
		<tr>
			<th colspan="2">
				<div class="pull-right">
					<?php echo icon('fa-eye').' '.$this->lang('views', $views, $views); ?>
				</div>
		...
		<tr>
			...
			<td class="text-left col-md-9">
				<div class="padding-top">
					...
					<a name="<?php echo $message_id; ?>"></a><?php echo icon('fa-clock-o').' '.time_span($date).' '.($last_message_read && $date <= $last_message_read ? icon('fa-comment-o').' '.$this->lang('message_read') : icon('fa-comment').' '.$this->lang('message_unread')); ?>
				</div>
				<hr />
