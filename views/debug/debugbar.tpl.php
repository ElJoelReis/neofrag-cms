<div class="debugbar hidden-xs<?php if ($active) echo ' active'; ?>">
	<div class="debugbar-resize"></div>
	<nav>
		<div class="logo hidden-sm"></div>
		<?php foreach ($tabs as $name => $tab): ?>
			<div class="debugbar-tab<?php if ($active == $name) echo ' active'; ?>" data-debugbar="<?php echo $name; ?>"><?php echo icon($tab[2]); ?><span class="hidden-sm hidden-md"> <?php echo $tab[1]; ?></span><?php if (!empty($tab[3])) echo ' '.$tab[3]; ?></div>
		<?php endforeach; ?>
		<div class="pull-right">
			...
			<div class="debugbar-close<?php if ($active) echo ' active'; ?>"><?php echo icon('fa-close'); ?></div>
		</div>
	</nav>
	<div class="debugbar-content"<?php if ($height = $this->session('debugbar', 'height')) echo ' style="height: '.max(200, $height).'px"'; ?>>
		<?php foreach ($tabs as $name => $tab): ?>
		<div class="debugbar-pane<?php if ($active == $name) echo ' active'; ?>" data-tab="<?php echo $name; ?>">
			<?php echo $tab[0]; ?>
		</div>
		<?php endforeach; ?>
	</div>
